...
This is not a simple problem as noted in the [Context, Literature, and Rationale], as both individual differences as well as musical features are difficult to quantify and subsequently model.
...
Work from from the field of linguistics has developed more sophisticated methodologies that are able to accommodate the above challenges and provide a more elegant way of handling these types of problems [@baayenMixedeffectsModelingCrossed2008].
...
@wolfGradesReflectDevelopment2014 noted multiple confounds in their study attempting to asses ability level in university musicians such as inflated grading, which led to ceiling effects, as well as a broad lack of consistency in how schools are assessing the success of their students. 
...
A fair amount of research from the music education literature examines melodic dictation in a more ecological setting [@buonviriEffectsMusicNotation2015; @buonviriEffectsPreparatorySinging2015; @buonviriEffectsSilenceSound2018; @buonviriEffectsTwoListening2017; @buonviriExplorationUndergraduateMusic2014; @buonviriMelodicDictationInstruction2015; @unsworthAutomatedVersionOperation2005], but most take a descriptive approach to modeling the results using between-subject manipulations.
...
This definition requires expert selection of what a chunk is, and does not take into account any of the Experimental factors put forward in the taxnomy presented in this dissertation's [Context, Literature, and Rationale].
...
Dictations were scored all melodies independently by two raters and then cross referenced for inter rater reliability ($\kappa$ = .96) whic suggests a high degree of inter-rater reliability [@kooGuidelineSelectingReporting2016].
...
In order to model the data and investigate the  hypotheses, I fit a series of linear mixed effects models using the R programming langauge [@R-base] and the _lme4_ and _LMERTEST_ [@batesFittingLinearMixedEffects2015] packages to predict scores from the dictation exercise using both individual and musical variables.
...
knitr::kable(
  ...
  caption = 'Series of Model Fits'
)
```

```{r metable1, echo=FALSE, fig.cap="",fig.align='center', out.width="100%"}
knitr::include_graphics("img/metable1.png")
```

```{r metable2, echo=FALSE, fig.cap="",fig.align='center', out.width="100%"}
knitr::include_graphics("img/metable2.png")
```

```{r metable3, echo=FALSE, fig.cap="",fig.align='center', out.width="100%"}
knitr::include_graphics("img/metable3.png")
```

```{r metable4, echo=FALSE, fig.cap="",fig.align='center', out.width="100%"}
knitr::include_graphics("img/metable4.png")
```
