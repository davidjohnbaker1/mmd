...
Some individuals perform exceptionally well, while others do not.
Presumably these differences in performance can be attributed to a number of factors.
One of the first, and perhaps most obvious, factors that might explain differences in ability would be an individual's musical training.

As noted in the previous chapter, people with some degree of musical training tend to outperform their less musically trained peers on many cognitive tasks.
A meta-analysis by Talamini and colleagues highlighted that these differences in memory are especially pronouced when the stimuli used in the experiments was tonal.
This finding suggests some sort of relationship between muscial training and an increase in memory for musical material.
...
One of the most common paradigms used in music perception, specifically the memory for melody literature, is same different tasks HALPERN.
Tasks like this require...
Note it is both rentention and manipulation bc you have to compare.
Can argue that this in some ways mirros MD without  the last two steps.
Have A and B.
Becuase the paradigm is set up like this, safte to assume it is a way to look at step 1 and 2 of dictation in general population.
And then can relate it to other variables of interest.

### Cognitive Measures of interest

Having established one of EF is WM, need to have way to measure it that is not direclty music and more established.
